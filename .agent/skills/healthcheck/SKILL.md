---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Scion deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Scion cron scheduling for periodic checks, or version status checks on a machine running Scion (laptop, workstation, Pi, VPS).
---

# Scion Host Hardening

## Overview

Assess and harden the host running Scion, then align it to a user-defined risk tolerance without breaking access. Use Scion security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Scion changes the host firewall, SSH, or OS updates; it does not.
...
5. Scion gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Scion can access sensitive data.
...
### 2) Run Scion security audits (read-only)

As part of the default read-only checks, run `agent security audit --deep`. Only offer alternatives if the user requests them:

1. `agent security audit` (faster, non-probing)
2. `agent security audit --json` (structured output)

Offer to apply Scion safe defaults (numbered):

1. `agent security audit --fix`

Be explicit that `--fix` only tightens Scion defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Scion version/update status (read-only)

As part of the default read-only checks, run `agent update status`.
...
- Credential hygiene notes (location of Scion creds, prefer disk encryption)
...
After Scion install or first hardening pass, run at least one baseline audit and version check:

- `agent security audit`
- `agent security audit --deep`
- `agent update status`

Ongoing monitoring is recommended. Use the Scion cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `agent cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `agent update status`
...
Before creating, `agent cron list` and match on exact `name`. If found, `agent cron edit <id> ...`.
If not found, `agent cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `agent update status` (preferred for source checkouts and channels)
2. `npm view scion version` (published npm version)

## Scion command accuracy

Use only supported commands and flags:

- `agent security audit [--deep] [--fix] [--json]`
- `agent status` / `agent status --deep`
- `agent health --json`
- `agent update status`
- `agent cron add|list|runs|run`

Do not invent CLI flags or imply Scion enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Scion compaction:
