...
metadata:
  { "scion": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/scion.sock"
SESSION=scion-python
...
- Use `SCION_TMUX_SOCKET_DIR`.
- Default socket path: `"$SCION_TMUX_SOCKET_DIR/scion.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `SCION_TMUX_SOCKET_DIR`).
