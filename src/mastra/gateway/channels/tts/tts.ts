...
import { normalizeChannelId } from "../plugins/index.js";
...
export function resolveTtsConfig(cfg: ScionConfig): ResolvedTtsConfig {
  const raw: TtsConfig = (cfg.messages?.tts ?? {}) as TtsConfig;
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.SCION_TTS_PREFS?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: ScionConfig): string | undefined {
  ...
  return [
    ...
    autoHint,
    `Keep spoken text ≤${maxLength} chars; longer text will be truncated.`,
...
function parseTtsDirectives(text: string, policy: ResolvedTtsModelOverrides): TtsDirectiveParseResult {
  if (!policy.enabled) {
