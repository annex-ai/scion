...
function prefersSips(): boolean {
  return (
    process.env.SCION_IMAGE_BACKEND === "sips" ||
    (process.env.SCION_IMAGE_BACKEND !== "sharp" && isBun() && process.platform === "darwin")
  );
}

function loadSharp(): (buffer: Buffer) => ReturnType<Sharp> {
  const sharp = (sharpModule as any).default ?? sharpModule;
  return (buffer) => (sharp as Sharp)(buffer, { failOnError: false });
}
...
async function withTempDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "scion-img-"));
  try {
    return await fn(dir);
  } finally {
