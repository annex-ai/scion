...
// @ts-nocheck — skipped: requires vitest (vi.mock, vi.mocked, vi.fn)
...
// Skipped: uses vi.mock, vi.mocked, vi.fn which have no direct bun:test equivalent
describe.skip("applyMediaUnderstanding (requires vitest)", () => {
  ...
  it("sets Transcript and replaces Body when audio transcription succeeds", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "scion-media-"));
  ...
  it("escapes file block content to prevent structure injection", async () => {
    ...
    expect(ctx.Body).toContain("&lt;/file&gt;");
    expect(ctx.Body).toContain("&lt;file");
    expect(((ctx.Body as string).match(/<\/file>/g) ?? []).length).toBe(1);
  });
