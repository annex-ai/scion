...
export function normalizeAttachments(ctx: MediaContext): MediaAttachment[] {
  ...
  const resolveMime = (count: number, index: number) => {
    ...
    return count === 1 ? (ctx.MediaType as string | undefined) : undefined;
  };
  ...
  const pathValue = (ctx.MediaPath as string | undefined)?.trim();
  const url = (ctx.MediaUrl as string | undefined)?.trim();
  if (!pathValue && !url) {
    return [];
  }
  return [
    {
      ...
      url: url || undefined,
      mime: ctx.MediaType as string | undefined,
      index: 0,
    },
  ];
}
...
export class MediaAttachmentCache {
  ...
  async getPath(params: {
    ...
    const tmpPath = path.join(os.tmpdir(), `scion-media-${crypto.randomUUID()}${extension}`);
