...
function resolveFileLimits(cfg: ScionConfig) {
  const files = (cfg as any).gateway?.http?.endpoints?.responses?.files;
...
export async function applyMediaUnderstanding(params: {
  ...
  const originalUserText =
    commandCandidates
      .map((value) => extractMediaUserText(value as string | undefined))
      .find((value) => value?.trim()) ?? undefined;
  ...
  try {
    ...
    if (decisions.length > 0) {
      ctx.MediaUnderstandingDecisions = [
        ...((ctx.MediaUnderstandingDecisions as MediaUnderstandingDecision[]) ?? []),
        ...decisions,
      ];
    }

    if (outputs.length > 0) {
      ctx.Body = formatMediaUnderstandingBody({ body: ctx.Body as string | undefined, outputs });
      ...
      ctx.MediaUnderstanding = [...((ctx.MediaUnderstanding as MediaUnderstandingOutput[]) ?? []), ...outputs];
    }
    const audioAttachmentIndexes = new Set(
      outputs.filter((output) => output.kind === "audio.transcription").map((output) => output.attachmentIndex),
    );
    ...
    if (fileBlocks.length > 0) {
      ctx.Body = appendFileBlocks(ctx.Body as string | undefined, fileBlocks);
    }
    if (outputs.length > 0 || fileBlocks.length > 0) {
      (finalizeInboundContext as any)(ctx, {
