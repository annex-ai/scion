...
export function buildProviderRegistry(overrides?: Record<string, MediaUnderstandingProvider>): ProviderRegistry {
  return buildMediaUnderstandingRegistry(overrides);
}

export function normalizeMediaAttachments(ctx: MediaContext): MediaAttachment[] {
...
async function resolveSherpaOnnxEntry(): Promise<MediaUnderstandingModelConfig | null> {
  ...
  return {
    ...
    args: [`--tokens=${tokens}`, `--encoder=${encoder}`, `--decoder=${decoder}`, `--joiner=${joiner}`, "{{MediaPath}}"],
  };
}
...
async function resolveKeyEntry(params: {
  ...
  const checkProvider = async (providerId: string, model?: string): Promise<MediaUnderstandingModelConfig | null> => {
...
function normalizeDeepgramQueryKeys(query: ProviderQuery): ProviderQuery {
  ...
  if ("detectLanguage" in result) {
    result.detect_language = result.detectLanguage as boolean;
    result.detectLanguage = undefined;
  }
  if ("smartFormat" in result) {
    result.smart_format = result.smartFormat as boolean;
    result.smartFormat = undefined;
  }
  // Strip undefined entries produced above
  return Object.fromEntries(Object.entries(result).filter(([, v]) => v !== undefined)) as ProviderQuery;
}
...
async function runProviderEntry(params: {
  ...
  if (capability === "image") {
    ...
    if (!provider?.describeImage) {
      throw new Error(`Image understanding provider "${providerId}" not available.`);
    }
    ...
    const model = entry.model?.trim() || DEFAULT_IMAGE_MODELS[providerId] || entry.model;
...
async function runCliEntry(params: {
  ...
  const outputDir = await fs.mkdtemp(path.join(os.tmpdir(), "scion-media-cli-"));
  const mediaPath = pathResult.path;
  const outputBase = path.join(outputDir, path.parse(mediaPath).name);

  const templCtx: MediaContext = {
    ...ctx,
    MediaPath: mediaPath,
