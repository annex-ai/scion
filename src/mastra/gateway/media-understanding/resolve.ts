...
export function resolvePrompt(capability: MediaUnderstandingCapability, prompt?: string, maxChars?: number): string {
...
export function resolveMaxBytes(params: {
  ...
  const configured =
    params.entry.maxBytes ?? params.config?.maxBytes ?? params.cfg.tools?.media?.[params.capability]?.maxBytes;
  if (typeof configured === "number") {
    return configured;
  }
...
export function resolveScopeDecision(params: {
  ...
  return resolveMediaUnderstandingScope({
    scope: params.scope,
    sessionKey: params.ctx.SessionKey as string | undefined,
    channel: (params.ctx.Surface ?? params.ctx.Provider) as string | undefined,
    chatType: normalizeMediaUnderstandingChatType(params.ctx.ChatType as string | null | undefined),
  });
}
...
export function resolveConcurrency(cfg: ScionConfig): number {
