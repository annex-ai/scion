...
// @ts-nocheck — skipped: requires vitest (vi.importActual, vi.doMock, vi.resetModules)
...
const HOME = path.join(realOs.tmpdir(), "scion-home-redirect");
...
// Skipped: uses vi.importActual, vi.doMock, vi.resetModules which have no bun:test equivalent
describe.skip("media store redirects (requires vitest)", () => {
  beforeAll(async () => {
  ...
  it("follows redirects and keeps detected mime/extension", async () => {
    ...
    mockRequest.mockImplementation((_url, _opts, cb) => {
      ...
      if (call === 1) {
        ...
        (res as any).headers = { location: "https://example.com/final" };
        setImmediate(() => {
          cb(res as unknown);
          res.end();
        });
