...
// Skipped: this file uses vitest imports and vitest-style dynamic module
// loading (import("./store.js")) which time out under `bun test`. The
// companion files store.header-ext.test.ts and store.redirect.test.ts have
// been converted to bun:test and cover the same store functionality.
describe.skip("media store", () => {
  let store: typeof import("./store.js");
  let home = "";
  const envSnapshot: Record<string, string | undefined> = {};

  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "SCION_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "scion-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.SCION_STATE_DIR = path.join(home, ".scion");
    ...
    await fs.mkdir(path.join(home, ".scion"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.scion${path.sep}media`);
  ...
  it("sets correct mime for xlsx by extension", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(saved.contentType).toBe("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
  ...
  describe("saveMediaBuffer with originalFilename", () => {
    it("embeds original filename in stored path when provided", async () => {
      await withTempStore(async (store) => {
        ...
        const saved = await store.saveMediaBuffer(buf, "text/plain", "inbound", 5 * 1024 * 1024, "report.txt");
    ...
    it("sanitizes unsafe characters in original filename", async () => {
      await withTempStore(async (store) => {
        ...
        const saved = await store.saveMediaBuffer(buf, "text/plain", "inbound", 5 * 1024 * 1024, "my<file>:test.txt");
    ...
    it("truncates long original filenames", async () => {
      await withTempStore(async (store) => {
        ...
        const saved = await store.saveMediaBuffer(buf, "text/plain", "inbound", 5 * 1024 * 1024, longName);
