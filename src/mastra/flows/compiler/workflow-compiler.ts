...
import { parseFlowFile } from "./flow-parser";
 ...
 */
/**
 * Compile a flow file to a Mastra workflow
 */
export function compileFlowToWorkflow(
  flowPath: string,
  options: CompileOptions,
): Workflow<any, any, any, any, any, any, any, any> {
  const flow = parseFlowFile(flowPath);
  return compileParsedFlow(flow, options);
}

/**
 * Compile a parsed flow to a Mastra workflow
 */
export function compileParsedFlow(
  flow: ParsedFlow,
  options: CompileOptions,
): Workflow<any, any, any, any, any, any, any, any> {
  const ast = parseMermaidFlowchart(flow.mermaidDiagram);
  // Convert ParsedFlow to ParsedSkill for internal compatibility
 ...
 */
export function compileParsedSkill(
  skill: ParsedSkill,
  options: CompileOptions,
): Workflow<any, any, any, any, any, any, any, any> {
 ...
 */
export function compileFlowAst(
  ast: FlowAst,
  skill: ParsedSkill,
  options: CompileOptions,
): Workflow<any, any, any, any, any, any, any, any> {
  const stepConfigs = new Map<string, StepConfig>();
  const steps = new Map<string, Step<any, any, any, any>>();
  ...
  buildExecutionGraph(workflow, ast, steps, stepConfigs, options);
...
function buildExecutionGraph(
  workflow: Workflow<any, any, any, any, any, any, any, any>,
  ast: FlowAst,
  steps: Map<string, Step<any, any, any, any>>,
  stepConfigs: Map<string, StepConfig>,
  options: CompileOptions,
): void {
...
function buildLoop(
  ...
  const bodySteps = loop.bodyNodes
    ...
    .filter((s): s is Step<any, any, any, any> => s !== undefined);
...
function buildBranch(
  ...
  const branches: Array<readonly [(ctx: { inputData: any }) => Promise<boolean>, Step<any, any, any, any>]> = [];
...
export function createSimpleWorkflow(
  ...
  return compileFlowAst(ast, skill, {
    id: flow.frontmatter.name.replace(/\s+/g, "-").toLowerCase(),
    ...options,
  });
}
